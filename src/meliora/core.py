...
All inputs are checked before calculation. Arrays are paired by position; caller
DataFrames are never mutated. See each function for its statistical convention.
"""
...
def _calibration(data, ratings, default_flag, predicted_pd):
    ...
    """
    v.frame(data, [ratings, default_flag, predicted_pd])
    y = v.binary(data[default_flag], default_flag)
    p = v.probabilities(data[predicted_pd], predicted_pd)
...
def herfindahl_test(data1, ratings, *, rating_order=None):
    ...
    """
    v.frame(data1, [ratings])
    labels = v.ordered_labels(data1[ratings], order=rating_order)
...
def herfindahl_multiple_period_test(data1, data2, ratings, alpha_level=0.05, *, rating_order=None):
    ...
    alpha = v.level(alpha_level)
    v.frame(data1, [ratings])
    v.frame(data2, [ratings])
    labels = v.ordered_labels(data1[ratings], data2[ratings], rating_order)
    if "total" in labels:
...
def jeffreys_test(data, ratings, default_flag, predicted_pd, alpha_level=0.05):
    ...
    posterior mass; Reject H0 uses p_value < alpha_level. This posterior tail is not an
...
def _classification(data, target, prediction):
    ...
    """
    v.frame(data, [target, prediction])
    return v.binary(data[target], target, both=True), v.vector(data[prediction], prediction)
...
def cumulative_lgd_accuracy_ratio(df, predicted_ratings, realised_outcomes, *, rating_order=None):
    ...
    """
    v.frame(df, [predicted_ratings, realised_outcomes])
    labels = v.ordered_labels(df[predicted_ratings], df[realised_outcomes], rating_order)
...
def _loss_arrays(ead, predicted, realised):
    ...
    """
    p, y = v.paired(predicted, realised, minimum=1, bounded=True)
    weights = v.vector(ead, "ead")
...
def information_value(df, feature, target, *, smoothing=0.5):
    ...
    """
    v.frame(df, [feature, target])
    v.binary(df[target], target, both=True)
    smooth = v.smoothing(smoothing)
...
def _paired_t(predicted, observed, *, alternative):
    ...
    """
    p, y = v.paired(predicted, observed, bounded=True)
    errors = y - p
...
def lgd_t_test(df, observed_lgd, expected_lgd, level="portfolio", segment_col=None):
    ...
    v.frame(df, [observed_lgd, expected_lgd] + ([] if level == "portfolio" else [segment_col]))
    groups = (
...
def _migration(data, initial, final, rating_order):
    ...
    """
    v.frame(data, [initial, final])
    labels = v.ordered_labels(data[initial], data[final], rating_order)
    return (
...
def population_stability_index(data, bin_flag, variable, *, expected=None, actual=None, smoothing=0.5):
    ...
    """
    v.frame(data, [bin_flag, variable])
    smooth = v.smoothing(smoothing)
    labels = v.ordered_labels(data[bin_flag])
    if len(labels) != 2:
...
def _entropy(data, realised_pd, count):
    ...
    """
    v.frame(data, [realised_pd, count])
    p = v.probabilities(data[realised_pd], realised_pd)
    w = v.vector(data[count], count)
...
def loss_shortfall(data, ead, predicted_lgd, realised_lgd):
    ...
    """
    v.frame(data, [ead, predicted_lgd, realised_lgd])
...
def normal_test(predicted_pd, realised_pd, alpha=0.05):
    ...
    """
    alpha = v.level(alpha)
    p, y = v.paired(predicted_pd, realised_pd, bounded=True)
    errors = y - p
...
def redelmeier_test(df, *, default_flag="DEFAULT_FLAG", first_pd="ADJUSTED_PD", second_pd="min_PD"):
    ...
    >>> assert 0 < result[1] < 1
    """
    v.frame(df, [default_flag, first_pd, second_pd])
    y = v.binary(df[default_flag], default_flag)
    p1 = v.probabilities(df[first_pd], first_pd)
    p2 = v.probabilities(df[second_pd], second_pd)

...
def stamp(language: str, payload: dict) -> Path:
    ...
    target.write_text(json.dumps({"meta": meta, "results": payload["results"]}, indent=1), encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("targets", nargs="+", choices=["data", "r", "sas", "matlab"])
    ...
    for target in args.targets:
        if target == "data":
            ...
            path.write_text(
                json.dumps({"meta": cases.python_meta(), "results": cases.to_records(results)}, indent=1),
                encoding="utf-8",
            )
